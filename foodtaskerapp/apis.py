...
def customer_add_order(request):
    """
        params:
            ...
            [{"meal_id": 1, "quantity": 2}, {"meal_id": 2, "quantity": 1}]
            stripe_token
        return:
            {"status": "success"}
    """
...
# RESTAURANT
##########
def restaurant_order_notification(request, last_requested_time):
    notification = Order.objects.filter(restaurant = request.user.restaurant,
        create_at__gt = last_requested_time).count()
...
def driver_pick_order(request):
    if request.method == "POST":
        # Get token
        access_token = access_token = AccessToken.objects.get(token = request.POST.get("access_token"),
            expires__gt = timezone.now())
        #Get driver
...
def driver_get_latest_order(request):
    # Get token
    access_token = access_token = AccessToken.objects.get(token = request.GET.get("access_token"),
        expires__gt = timezone.now())
    driver = access_token.user.driver
    order = OrderSerializer(
